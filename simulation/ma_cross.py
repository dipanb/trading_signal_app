...
class MAResult:
    ...
    def result_ob(self):
        return dict(
            pair = self.pairname,
            num_trades = int(self.df_trades.shape[0]),
...
def process_results(results_list,filepath):
    ...
    process_macro(results_list,get_fullname(filepath,"ma_"))
    # rl = [x.result for x in results_list]
    # df = pd.DataFrame.from_dict(rl)
...
def analyse_pair(instrument, granularity, ma_long,ma_short,filepath):
    ...
    for ma_l in ma_long:
        for ma_s in ma_short:
            ...
            ma_result = assess_pair(price_data,get_ma_col(ma_l),get_ma_col(ma_s),granularity)
            print(ma_result)
